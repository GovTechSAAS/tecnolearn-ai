...
export default function ChamadaPage() {
  ...
  useEffect(() => {
    async function fetchClasses() {
      ...
      const { data } = await supabase.from('classes').select('id, name').order('created_at', { ascending: false });
      setClasses(data || []);
      setLoading(false);
    }
    fetchClasses();
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const endDate = `${selectedMonth}-31`; // Simplificado, Postgres lida com o range
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          date,
          class:classes(name),
          entries:attendance_entries(status, student:profiles(full_name))
        `)
        .gte('date', startDate)
        .lte('date', endDate)
                  ...
                  <Card key={record.id} className="hover:shadow-lg transition-all border-border/40 group overflow-hidden">
                    <CardHeader className="p-4 bg-zinc-50 dark:bg-zinc-900/40 border-b border-border/20">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="text-[10px] font-bold text-muted-foreground">{new Date(record.date).toLocaleDateString('pt-BR')}</span>
                      </div>
      ...
      {viewingRecord && (
        <div className="fixed inset-0 bg-black/60 shadow-2xl backdrop-blur-md z-50 flex items-center justify-center p-4">
           <Card className="w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              <CardHeader className="bg-zinc-50 dark:bg-zinc-900 border-b border-border">
                 <div className="flex justify-between items-center">
                    <div>
                       ...
                       <CardDescription>{viewingRecord.class?.name} - {new Date(viewingRecord.date).toLocaleDateString('pt-BR')}</CardDescription>
                    </div>
              ...
              <CardFooter className="bg-zinc-50 dark:bg-zinc-900 border-t border-border p-4 flex justify-between">
                 <p className="text-xs text-muted-foreground">ID do Registro: {viewingRecord.id}</p>
